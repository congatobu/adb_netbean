/*
 v * Copyright (C) 2015 Pedro Vicente Gómez Sánchez.
 *
...
public class ADBParser {
	private static final String TRANSPORT_INDICATOR = "transport";
...
//				continue;
//			}
			String type = parseDeviceType(line);
	...
	public String parseAdbServiceTcpPort(String getPropOutput) {
		...
		Matcher portMatcher = Pattern.compile("(?<=\\[(service\\.adb\\.tcp\\.port).: \\[)([^\\]]+)(?=\\])")
	...
	private String parseDeviceType(String line) {
		int start = line.indexOf(DEVICE_INDICATOR) + DEVICE_INDICATOR.length();
		int end = line.indexOf(TRANSPORT_INDICATOR) - 1;
		if (end < 0) {
			end = line.length();
		}
		return line.substring(start, end);
